...
void LoadingLevel_BackDrop::Start()
{
	...
	// BackDrop
	BackDropRenderer_ = CreateTransformComponent<GameEngineUIRenderer>(static_cast<int>(RenderOrder::BACKDROP));
	BackDropRenderer_->SetImage("LoadingLevel_BackDrop.png");
	BackDropRenderer_->GetTransform()->SetWorldScaling(WindowSize);

	// Logo
	LogoRenderer_ = CreateTransformComponent<GameEngineUIRenderer>(static_cast<int>(RenderOrder::UI));
	...
	// Bottom Panel
	BottomPanelRenderer_ = CreateTransformComponent<GameEngineUIRenderer>(static_cast<int>(RenderOrder::UIPANEL0));
